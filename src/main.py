...
@app.get("/books")
async def get_books():
    res = cursor.execute("select title from book order by book.id")
    books = res.fetchall()
    return [title for [title] in books]

@app.get("/{book}/chapter_count")
...
@app.get("/{book}/{chapter}/verse_count")
...
async def get_random_verse():
    ...
    chapter = randint(1, count)
    count = await get_verses_count(book, chapter)
    verse = randint(1, count)
